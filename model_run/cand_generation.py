...
from numpy import exp, log, dot, mean, zeros, outer, random, dtype, float32 as REAL,\
...
w_indices = [x for x in w_indices if x is not None]
d_indices = [x for x in d_indices if x is not None]
...
model1.dv.vectors_docs_norm = (model1.dv.vectors / sqrt((model1.dv.vectors ** 2).sum(-1))[..., newaxis]).astype(REAL)[d_indices]
...
def get_labels(topic_num):
    ...
    for item in topic_list[topic_num]:
        try:
            ...
            tempdoc2vec = model1.syn0norm[model1.wv.key_to_index[item]]
        except:
            pass
        else:
            ...
            distsdoc2vec = dot(model1.dv.vectors_docs_norm, meandoc2vec)
        ...
        try:
            ...
            tempword2vec = model2.syn0norm[model2.wv.key_to_index[item]]
        except:
            pass
        else:
    ...
    for elem in bestdoc2vec:
        ind = d_indices[elem]
        temp = model1.dv.index_to_key[ind].decode('utf-8')
    ...
    for element in bestword2vec:
        ...
        temp = model2.wv.index_to_key[ind]
    ...
    for elem in comb_labels:
        try:
          ...
          newlist_doc2vec.append(d_indices.index(
              model1.dv.key_to_index[elem.encode('utf-8')]))
          temp = get_word(elem)
          # newlist_word2vec.append(w_indices.index(model2.wv.vocab[temp].index))
          newlist_word2vec.append(w_indices.index(model2.wv.key_to_index[temp]))
        except:
            pass
    ...
    resultlist_doc2vecnew = [(model1.dv.index_to_key[
        d_indices[elem]], float(avgdoc2vec[elem])) for elem in newlist_doc2vec]
    resultlist_word2vecnew = [(model2.wv.index_to_key[w_indices[elem]], float(
        avgword2vec[elem])) for elem in newlist_word2vec]
    
    resultlist_doc2vecnew = [(item.decode('utf-8'), prob) for item, prob in resultlist_doc2vecnew]
